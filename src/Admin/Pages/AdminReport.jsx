...
import { getreportsApi, reportResolveApi } from "../../services/allApi";
...
const AdminReport = () => {
  ...
  const fetchReviews = async () => {
    try {
      ...
      if (response.data.success) {
        setReviews(response.data.reportedReviews);
        setFilteredReviews(response.data.reportedReviews);
      }
    } catch (err) {
  ...
  const handleSubmitReport = async () => {
    try {
      const response = await reportResolveApi(currentReview._id, reportReason);
      console.log(response);
  ...
  const handleResolveReport = async (reviewId, action) => {
    try {
      const response = await reportResolveApi(reviewId, action);
      if (response.data.success) {
        if (action === "delete") {
                    ...
                    >
                      ...
                      <Stack direction="row" spacing={2} sx={{ mt: 1 }}>
                        <Button
                          ...
                          size="small"
                          onClick={() =>
                            handleResolveReport(review._id, "delete")
                          }
                        >
                          Delete
                        </Button>
                        <Button
                          ...
                          size="small"
                          onClick={() =>
                            handleResolveReport(review._id, "keep")
                          }
                        >
                          Keep
                        </Button>
